...
export const HologramDevice = () => {
  return (
    <group position={[0, -GLOBE_RADIUS * 1.5, 0]} rotation={[Math.PI / 12, 0, 0]}>
      ...
      {/* Screen / Projector Source Area */}
      <mesh position={[0, -0.015, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[1.1, 2.1]} />
        <meshBasicMaterial color="#002222" transparent opacity={0.2} blending={THREE.AdditiveBlending} />
      </mesh>

      {/* Volumetric Emission - Soft radial haze at the source */}
      ...
      {/* Subtle floating bloom under the globe center */}
      <group position={[0, GLOBE_RADIUS * 0.7, 0]}>
        <mesh scale={[GLOBE_RADIUS * 0.7, 0.05, GLOBE_RADIUS * 0.7]}>
          <sphereGeometry args={[1, 32, 16]} />
          <meshBasicMaterial 
            color="#00ffff" 
            transparent 
      ...
      {/* Soft Light Casting */}
      <pointLight position={[0, 0.3, 0]} intensity={0.4} distance={3} color="#00ffff" />
    </group>
  );
};
  ...
  useFrame((state) => {
    if (pointsRef.current) {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        // Subtle drift
        positionsArr[i * 3] += Math.sin(state.clock.elapsedTime + i) * 0.001;
        
        if (positionsArr[i * 3 + 1] > 2.5) {
          positionsArr[i * 3 + 1] = 0;
          positionsArr[i * 3] = (Math.random() - 0.5) * 0.8;
        }
      }
